...
If you'd like to remove a course at any time, you can use `uninstall_course("Course Name Here")` or `uninstall_all_courses()` to start fresh.  If there is a issue with a specific topic use the default "R Programming: The basics of programming" and send the `savehistory(file = "topic_name.txt")` to emiedema@jessup.edu to indicate completion.